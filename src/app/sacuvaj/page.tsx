...
export default function SacuvajPage() {
  const router = useRouter();
  const { user } = useAuth();
  const { items, removeItem, clearSaved, savedCount } = useSaved();
  ...
  const handleClear = () => {
    clearSaved();
    setProducts([]);
    showToast(t('saved.cleared'));
  };

  const handleContactSeller = async (e: React.MouseEvent, product: ProductFull) => {
    e.stopPropagation();
    if (!user) { router.push('/login?redirect=/sacuvaj'); return; }
    ...
    try {
      ...
      showToast(t('product.contactError'), 'error');
    } finally {
  ...
  return (
    <MainLayout title={t('saved.title')} showSigurnost={false}>
      <div className="pt-2 pb-24 px-2 sm:px-0">

        {/* Header */}
        <div className="mb-6 px-1 sm:px-2">
          <h1 className="text-2xl font-black text-[var(--c-text)] uppercase tracking-tight mb-1">{t('saved.title')}</h1>
          <p className="text-xs text-[var(--c-text3)]">{t('saved.itemCount', { count: savedCount })}</p>
        </div>
        ...
        {loading ? (
          ...
          <>
            ...
            {/* Hint — temporär, kein Shop-Feeling */}
            <div className="w-full bg-[var(--c-hover)] border border-[var(--c-border)] rounded-[24px] py-4 px-5 flex items-center gap-4 mb-3">
              <div className="w-10 h-10 rounded-full bg-[var(--c-active)] flex items-center justify-center shrink-0">
                <i className="fa-regular fa-bookmark text-[var(--c-text3)]"></i>
              </div>
              <div>
                <p className="text-[11px] font-bold text-[var(--c-text2)]">{t('saved.hint')}</p>
              </div>
            </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 opacity-50">
            <div className="w-20 h-20 bg-[var(--c-hover)] rounded-full flex items-center justify-center mb-4">
              <i className="fa-regular fa-bookmark text-3xl text-[var(--c-text3)]"></i>
            </div>
            <p className="text-sm text-[var(--c-text2)] font-bold uppercase tracking-wider">{t('saved.empty')}</p>
